...
class mainWindow(QMainWindow, form_class) :
    ...
    def addBlock(self, blkCode, name, pos):
        ...
        if self.getCode(block) in [blockCode.INDENT,blockCode.FOR]:
    ...
    def setDepth(self, blkCode, depth):
        ...
        for block in self.blocks[blkCode].blockList:
            if self.getCode(self.blocks[block]) in [blockCode.INDENT,blockCode.FOR]:
    ...
    def setPos(self, blkCode, pos):
        ...
        for i in range(len(self.blocks[blkCode].blockList)):
            if self.getCode(self.blocks[self.blocks[blkCode].blockList[i]]) in [blockCode.INDENT,blockCode.FOR]:
    ...
    def maxDepthInIndent(self, blkCode):
        ...
        for i in self.blocks[self.ancestor(blkCode)].blockList:
            if not (self.getCode(self.blocks[i]) in [blockCode.INDENT,blockCode.FOR]):
                continue
    ...
    def dropEvent(self, e: QDropEvent):
        ...
        if dropT == dropType.INDENT:
            ...
            if self.getCode(self.blocks[code].parent()) in [blockCode.INDENT,blockCode.FOR]:
            ...
            if self.getCode(self.blocks[code]) in [blockCode.INDENT,blockCode.FOR]:
            ...
            self.blocks[code].setParent(self.centralwidget)
            if self.getCode(self.blocks[code]) in [blockCode.INDENT,blockCode.FOR]:
                for i in range(len(self.indentPos)):
                    self.setDepth(code, 0)
